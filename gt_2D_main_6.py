...
makedir(PATH)
shutil.copy('gt_2D_main_6.py',PATH)
...
def calculate_overall_lwlrap_sklearn(truth, scores):
    ...
    # truth = K.eval(K.constant(truth, dtype='float32'))
    # scores = K.eval(K.variable(scores, dtype='float32'))
...
class fft_1Dconv(object):
    ...
    def _built_model(self):
        ...
        #
        # x = self.conv1d_bn(x, 32, 51, strides=2, padding='same')
        # x = self.conv1d_bn(x, 64, 9, strides=2, padding='same')
        ...
        inputs = Input(shape=self._input_shape)
        x = Reshape((SubSample, 1))(inputs)
        # x = Reshape((step_input_size, NFFT, 1))(inputs)
        x_real = Conv1D(fft_number, NFFT, strides=(NFFT-noverlap),padding='valid', activation=None, use_bias=False,
                        kernel_initializer=self.my_init_real, name='my_conv_real', trainable=False)(x)
        x_real = Lambda(self.divide_NFFT, output_shape=self.antirectifier_output_shape)(x_real)
        x_real = Reshape((step_input_size,fft_number, 1))(x_real)
        # x_real = Lambda(lambda x: tf.transpose(x, [0, 3, 1, 2]))(x_real)
        # x_real = Lambda(self.log_lbd, output_shape=self.antirectifier_output_shape)(x_real)

        x_imag = Conv1D(fft_number, NFFT, strides=(NFFT-noverlap), padding='valid', activation=None, use_bias=False,
                        kernel_initializer=self.my_init_imag, name='my_conv_imag', trainable=False)(x)
        x_imag = Lambda(self.divide_NFFT, output_shape=self.antirectifier_output_shape)(x_imag)
        x_imag = Reshape((step_input_size, fft_number, 1))(x_imag)
        # x_imag = Lambda(lambda x: tf.transpose(x, [0, 3, 1, 2]))(x_imag)
        # x_imag = Lambda(self.log_lbd, output_shape=self.antirectifier_output_shape)(x_imag)

        x_real2 = merge([x_real, x_real], mode='mul')
        x_imag2 = merge([x_imag, x_imag], mode='mul')
        x = merge([x_real2, x_imag2], mode='sum')
        x = Lambda(self.power_f, output_shape=self.antirectifier_output_shape)(x)
        # x = Lambda(self.antirectifier, output_shape=self.antirectifier_output_shape)(x)
        x = Lambda(self.log_lbd, output_shape=self.antirectifier_output_shape)(x)
        # x= Dropout(0.95)(x)
        # x = Activation('tanh')(x)
        # x = Conv2D(20, (5, 5), padding='same', activation='relu', name='my_conv2d_1', trainable=True)(x)
        # x = MaxPooling2D((2, 2), strides=(2, 2), padding='same')(x)
        # x = Conv2D(20, (5, 5), padding='same', activation='relu', name='my_conv2d_2', trainable=True)(x)
        # x = MaxPooling2D((2, 2), strides=(2, 2), padding='same')(x)
        # x = Conv2D(20, (5, 5), padding='same', activation='relu', name='my_conv2d_3', trainable=True)(x)
        # x = Flatten()(x)
        # x = Dense(500, activation='relu', name='my_dense_0', trainable=True)(x)
        # x = Dropout(0.5)(x)
        # output = Dense(nb_classes, activation='sigmoid', name='my_dense_1', trainable=True)(x)
        # model = Model(inputs=inputs, outputs=output)
        # return model

        x = Conv2D(128, (5, 5), padding='same', activation=self.my_activation, name='my_conv2d_1', trainable=True)(x)
        # x = LeakyReLU()(x)
        x = MaxPooling2D((2, 2), strides=(2, 2), padding='same')(x)

        x = Conv2D(128, (3, 3), padding='same', activation=self.my_activation, name='my_conv2d_2', trainable=True)(x)
        # x = LeakyReLU()(x)
        x = MaxPooling2D((2, 2), strides=(2, 2), padding='same')(x)

        x = Conv2D(128, (3, 3), padding='same', activation=self.my_activation, name='my_conv2d_3', trainable=True)(x)
        # x = LeakyReLU()(x)
        x = MaxPooling2D((2, 2), strides=(2, 2), padding='same')(x)

        x = Conv2D(64, (3, 3), padding='same', activation=self.my_activation, name='my_conv2d_4', trainable=True)(x)
        # x = LeakyReLU()(x)
        x = MaxPooling2D((2, 2), strides=(2, 2), padding='same')(x)

        x = Conv2D(64, (3, 3), padding='same', activation=self.my_activation, name='my_conv2d_5', trainable=True)(x)
        # x = LeakyReLU()(x)
        x = MaxPooling2D((2, 2), strides=(2, 2), padding='same')(x)

        x = Flatten()(x)
        x = Dense(512, activation='relu', name='my_dense_0', trainable=True)(x)
        x = L.ActivityRegularization()(x)
        x = Dropout(0.5)(x)
        output = Dense(nb_classes, activation='softmax', name='my_dense_1', trainable=True)(x)
        model = Model(inputs=inputs, outputs=output)
        return model
    ...
    def my_init_real(shape, name=None):
        ...
        for k in range(fft_number):
            for i in range(NFFT):
                w_matrix[i, 0, k] = cos(2 * k * pi * (i / Pad_to))
    ...
    def my_init_imag(shape, name=None):
        ...
        for k in range(fft_number):
            for i in range(NFFT):
                w_matrix[i, 0, k] = -sin(2 * k * pi * (i / Pad_to))
        return K.variable(w_matrix, name=name)

    @staticmethod
    def my_activation(x, alpha=-0.01):
